...
One-click toggle, persisted in `localStorage`. Warm off-white paper, rich black ink, deep indigo accent. Same component vocabulary, fully theme-aware charts.

![Executive Dashboard — Porcelain (light theme)](docs/images/ui_dashboard_light.png)

### Design system

Two sophisticated modern themes that share the same token namespace and component vocabulary:

- **Obsidian** (dark) — deep ink canvas (#0a0a0b), electric-violet signature (#8b5cf6), cool neutral surfaces. Refined and premium.
- **Porcelain** (light) — warm off-white paper (#fafaf7), deep indigo signature (#4338ca), rich black ink (#1c1917). Editorial and institutional.

Type: **Inter** for prose, **JetBrains Mono** for all numerics, codes, pills, and chrome — with tabular-num spacing so columns of numbers line up cleanly. Server identity (**CRM · ERP · QA · Analytics**) is encoded as small colored mono pills used consistently across the tool catalog, the agent trace, and the data-model docs. The wordmark uses an animated accent gradient that sweeps across the letters every 8 s — a subtle nod to live data flow.
